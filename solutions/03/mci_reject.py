...
def dlogn(x,bounds):
    return bounds[1]*n*exp(-0.5*(np.log(x)**2)/(x*np.sqrt(2.0*np.pi))
...
def reject(fDTAR,fDPRO,fRPRO,R,bounds_dst,bounds_val):
    ...
    else: # fast version for application: tries done in blocks
        while (Ratt < R): # to speed this up, we generate attempts of length R
                          # and repeat until Ratt == R. Otherwise, we'd have a
                          # while loop stepping through each try - very inefficient.
            x                  = fRPRO(R,bounds_dst) 
            u                  = np.random.rand(R)
            Qx                 = fDPRO(x,bounds_val)
            Px                 = fDTAR(x,np.array([0.0,1.0]))
            dRatt              = (np.less_equal(u,Px/Qx)).sum()            # number of new accepted points.
            dR                 = np.min(np.array([dRatt,R-Ratt])) # don't overshoot
            if (N > 1):
                xr[Ratt:Ratt+dR,:] = np.squeeze(x[np.where(u <= Px/Qx),:])    # accepted points. These are the {x_r}.
            else:
                xr[Ratt:Ratt+dR]   = (np.squeeze(x[(np.where(u <= Px/Qx))]))[0:dR]
            Ratt               = Ratt + dR                              # keep track of accepted number
