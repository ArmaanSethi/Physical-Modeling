...
def bnc_nocharge(q):#changed u to q?
    J      =  q.size-2
    q[0  ] = -q[1]
    q[J+1] = -q[1]
    return q
...
def ftcs(x,t,alpha,fBNC,fINC):
    #.....
    q = np.zeros((x.size+2, t.size))
    q[1:-1,0] = fINC(x)
    q[:,0] = fBNC(q[:,0])

    for i in range(len(t)-1):
        q[1:-1,i+1] = q[1:-1,i] - (alpha/2)*(q[2:,i]-q[:-2,i])
        q[:,i+1] = fBNC(q[:,i+1])
...
def lax(x,t,alpha,fBNC,fINC):
    ...
    for i in range(len(t)-1):
        q[1:-1,i+1] = (0.5)*(q[:-2,i]+q[2:,i]) - (alpha/2)*(q[2:,i]-q[:-2,i])
        q[:,i+1] = fBNC(q[:,i+1])
...
def leapfrog(x,t,alpha,fBNC,fINC):
    ...
    for i in range(len(t)-1):
        if i == 0:
            q[1:-1,i+1] = q[1:-1,i] - alpha *(q[1:-1,i]- q[:-2,i])
            q[:,i+1] = fBNC(q[:,i+1])
        else:
            q[1:-1,i+1] = q[1:-1,i-1] - alpha*(q[2:,i] -q[:-2,i])
            q[:,i+1] = fBNC(q[:,i+1])
...
def laxwendroff(x,t,alpha,fBNC,fINC):
    ...
    for i in range(len(t)-1):
        q[1:-1,i+1] = q[1:-1,i] - 0.5*alpha * ((q[2:,i] + q[1:-1,i]) - alpha *(q[2:,i] - q[1:-1,i]) - (q[1:-1, i] + q[:-2,i]) + alpha * (q[1:-1, i] - q[:-2,i] ) )
        q[:,i+1] = fBNC(q[:,i+1])
...
def advection_solve(J,minmaxt,minmaxx,cfl,fSOL,fINC,fBNC):
    ...
    N     = int((minmaxt[1]-minmaxt[0])/dt)+1 #added one to correct value of N
